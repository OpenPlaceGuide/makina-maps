...
Prepare import by downloading OpenStreetMap data and setting up the configuration for an area. By default the provider is [Geofabrik](http://download.geofabrik.de/); use the `PROVIDER` environment variable to pick another source. For openstreetmap.fr, area IDs are hierarchical (e.g. `africa/ethiopia`). Extracts are stored in the `data` directory.

```
# Geofabrik example
../scripts/20-import-prepare.sh andorra

# openstreetmap.fr example (Ethiopia) with minutely updates
PROVIDER=osmfr ../scripts/20-import-prepare.sh africa/ethiopia

# For openstreetmap.fr extracts you may also enable broken-reference cleanup:
BORDERS_CLEANUP=true PROVIDER=osmfr ../scripts/20-import-prepare.sh africa/ethiopia
```
...
The scripts `20-import-prepare.sh` and `30-import-extract.sh` can be replayed with the same or another area.
When `DIFF_MODE=true` (the default in `.env`), `20-import-prepare.sh` also creates a
`data/<area>.repl.json` with the correct replication URL; for `osmfr` this is the
openstreetmap.fr **minutely** replication stream.

### Update OpenMapTiles data

From the `openmaptiles` directory.

Run the updater. It loops over pending updates, then waits for new updates.
```
...
> **Note:** updates rely on the replication config generated by `20-import-prepare.sh`.
> For `PROVIDER=osmfr` this config points at the openstreetmap.fr **minutely** diff
> stream, so updates are applied roughly once per minute.


### Root Makefile helpers

A root `Makefile` is provided as a convenience so you can drive the whole stack
from the project root without changing directories. The same Ethiopia example as
above becomes:
